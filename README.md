...
- 对于事件源派发：GTAV-Ops遵循单线程架构，这意味着：你不可能通过快速触发事件源，来实现同一个宏在两个线程上并行执行，例如车辆点火
- 对于宏行为的执行（本质上是Action执行）：
  - 对于一般执行宏：GTAV-Ops基于单线程模型运行，这意味着两个`Action`不可能同时执行
...
`club.pineclone.gtavops.macro.trigger.source.InputSource.java`是所有事件源的父类，它定义了包括：
