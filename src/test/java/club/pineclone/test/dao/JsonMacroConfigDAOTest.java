...
public class JsonMacroConfigDAOTest {
    ...
    private final AppConfig appConfig = new AppConfig(PathUtils.getAppHomePath());

    private final MacroEntryMapper macroEntryMapper = Mappers.getMapper(MacroEntryMapper.class);
    ...
    public void beforeTest() throws Exception {
        ((JsonMacroEntryDAO) macroEntryDao).onAppInit();  /* 初始化 宏实例 数据库 */
        ((JsonMacroConfigDAO) macroConfigDao).onAppInit();  /* 初始化 宏配置 数据库 */
    }
    ...
    public void insertBatch() {
        ...
        MacroConfig macroConfig2 = macroConfigMapper.copy(macroConfig1);
