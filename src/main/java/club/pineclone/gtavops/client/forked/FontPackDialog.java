...
public class FontPackDialog<T> {
    ...
    public FontPackDialog() {
        this(new VStage());
    }

    public FontPackDialog(VStage vStage) {
        this.vStage = vStage;
...
//        FXUtils.observeWidth(vStage.getRootSceneGroup().getNode(), this.content);
    ...
    public void setButtons(List<ForkedDialogButton<T>> buttons) {
        ...
        for (var btn : buttons) {
            var text = btn.textProperty().get();
            var button = new FusionButton(text);
    ...
    /* 确认取消弹窗 1: 确认  0: 取消 */
    public static FontPackDialog<Integer> simpleDialog() {
        return simpleDialog(null, Modality.NONE);
    }

    public static FontPackDialog<Integer> simpleDialog(String text) {
        return simpleDialog(text, Modality.NONE);
    }

    public static FontPackDialog<Integer> simpleDialog(Modality modality) {
        return simpleDialog(null, modality);
    }

    public static FontPackDialog<Integer> simpleDialog(String text, Modality modality) {
        FontPackDialog<Integer> dialog = createConfirmCancelDialog();
        dialog.setHeaderText(text);
        dialog.getVStage().getStage().initModality(modality);
        return dialog;
    }

    public static FontPackDialog<Integer> simpleDialog(String text, Modality modality, int flag) {
        FontPackDialog<Integer> dialog = createSimpleDialog(flag);
        dialog.setHeaderText(text);
        dialog.getVStage().getStage().initModality(modality);
        return dialog;
    }

    private static FontPackDialog<Integer> createConfirmCancelDialog() {
        return createSimpleDialog(CONFIRM | CANCEL);
    }

    private static FontPackDialog<Integer> createSimpleDialog(int flag) {
        return createDialog(flag);
    }

    @Deprecated
    private static FontPackDialog<Integer> createDialog(int flags) {
        FontPackDialog<Integer> dialog = new FontPackDialog<>(new VStage(
        ...
        if ((flags & CONFIRM) != 0) {
            buttons.add(new ForkedDialogButton<>("i18n.common.confirm", CONFIRM));
        }

        if ((flags & CANCEL) != 0) {
            buttons.add(new ForkedDialogButton<>("i18n.common.cancel", CANCEL));
        }
    ...
    public static FontPackDialog<Integer> stackTraceDialog(Throwable throwable, int flags) {
        return stackTraceDialog("", throwable, flags);
    }

    public static FontPackDialog<Integer> stackTraceDialog(Throwable throwable) {
        return stackTraceDialog("", throwable, CONFIRM | CANCEL);
    }

    public static FontPackDialog<Integer> stackTraceDialog(String desc, Throwable throwable) {
        return stackTraceDialog(desc, throwable, CONFIRM | CANCEL);
    }

    public static FontPackDialog<Integer> stackTraceDialog(String desc, Throwable throwable, int flags) {
        Logger.error(LogType.ALERT, "StackTraceAlert: " + desc, throwable);
        FontPackDialog<Integer> dialog = createDialog(flags);
